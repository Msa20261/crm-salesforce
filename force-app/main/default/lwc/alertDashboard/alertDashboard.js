...
export default class AlertDashboard extends LightningElement {
  ...
  get alertes() {
    ...
    return this._wiredResult.data.map((a) => ({
      ...
      cssCard:
        "alert-card statut-" +
        (a.Statut__c || "").toLowerCase().replace("é", "e").replace("è", "e"),
      cssType: "tag-type " + (TYPE_COLORS[a.Type_Alerte__c] || "type-gray"),
      cssStatut:
        "tag-statut statut-tag-" +
        (a.Statut__c || "").toLowerCase().replace("é", "e").replace("è", "e"),
      dateFormatee: a.Date_Alerte__c
        ? new Date(a.Date_Alerte__c).toLocaleDateString("fr-FR", {
            day: "2-digit",
            month: "short",
            year: "numeric"
          })
      ...
      seuilStyle:
        "width:" +
        Math.min(a.Seuil_Depasse__c || 0, 100) +
        "%;background:" +
        this._seuilColor(a.Seuil_Depasse__c),
  ...
  get btnTousClass() {
    return "filter-btn" + (this.filtre === "tous" ? " active" : "");
  }
  get btnNouvelleClass() {
    return "filter-btn" + (this.filtre === "nouvelle" ? " active" : "");
  }
  get btnVueClass() {
    return "filter-btn" + (this.filtre === "vue" ? " active" : "");
  }
  get btnResolueClass() {
    return "filter-btn" + (this.filtre === "resolue" ? " active" : "");
  }

  // Actions
  marquerVue(evt) {
    this._updateStatut(evt.currentTarget.dataset.id, "Vue");
  }
  resoudre(evt) {
    this._updateStatut(evt.currentTarget.dataset.id, "Résolue");
  }
