...
jest.mock('@salesforce/apex/CrmDashboardController.getAlertes', () => ({ default: jest.fn() }), { virtual: true });
jest.mock('@salesforce/apex/CrmDashboardController.updateStatutAlerte', () => ({ default: jest.fn() }), { virtual: true });
...
describe('c-alert-dashboard', () => {
    ...
    it('affiche les alertes quand le wire retourne des données', async () => {
        ...
        getAlertes.emit(MOCK_ALERTES);
        await Promise.resolve();

        // Le getter alertes doit mapper les données
        const cards = element.shadowRoot.querySelectorAll('.alert-card');
        // En fonction du rendu : si les cartes existent, il y en a 3
        if (cards.length > 0) {
    ...
    it('filtre les alertes par statut "Nouvelle"', async () => {
        ...
        // Cherche le bouton de filtre "Nouvelle"
        const filterBtn = element.shadowRoot.querySelector('.filter-btn[data-filter="nouvelle"], [onclick*="filtrerNouvelle"]');
        if (filterBtn) {
            filterBtn.click();
            await Promise.resolve();
            // Après filtre : seule l'alerte "Nouvelle" doit être visible
            const cards = element.shadowRoot.querySelectorAll('.alert-card');
            if (cards.length > 0) expect(cards.length).toBe(1);
        }
        expect(element).toBeTruthy();
    });
    ...
    it('calcule la couleur du seuil selon le pourcentage', async () => {
        ...
        // Alerte avec seuil >= 100 → rouge
        getAlertes.emit([{ ...MOCK_ALERTES[0], Seuil_Depasse__c: 110 }]);
        await Promise.resolve();
    ...
    it('appelle updateStatutAlerte au clic sur "Marquer Vue"', async () => {
        ...
        const marquerVueBtn = element.shadowRoot.querySelector('[data-id="a001"][onclick*="marquerVue"], .btn-vue');
        if (marquerVueBtn) {
            marquerVueBtn.click();
            await Promise.resolve();
            expect(updateStatutAlerte).toHaveBeenCalledWith(
                expect.objectContaining({ alerteId: 'a001', statut: 'Vue' })
            );
        }
        expect(element).toBeTruthy();
    });
    ...
    it('passe en état d\'erreur si le wire échoue', async () => {
        ...
        getAlertes.error({ message: 'Erreur serveur' });
        await Promise.resolve();
