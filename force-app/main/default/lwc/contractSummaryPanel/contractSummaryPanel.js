...
export default class ContractSummaryPanel extends LightningElement {
  ...
  get contrat() {
    return this._summary?.contrat || {};
  }
  get contractNumber() {
    return this.contrat.ContractNumber || "";
  }
  get contractStatus() {
    return this.contrat.Status || "";
  }
  ...
  get nbLignes() {
    return this._summary?.nbLignes ?? 0;
  }
  get nbAlertes() {
    return this._summary?.nbAlertes ?? 0;
  }
  get nbFactures() {
    return this._summary?.nbFactures ?? 0;
  }
  get hasAlertes() {
    return this.nbAlertes > 0;
  }

  get totalConsoFmt() {
    return fmt(this._summary?.totalConso);
  }
  get totalAcheteFmt() {
    return fmt(this._summary?.totalAchete);
  }
  get montantFactures() {
    return fmt(this._summary?.montantFactures);
  }

  get montantAbonnement() {
    return fmt(this.contrat.Montant_Abonnement__c);
  }
  get montantToken() {
    return fmt(this.contrat.Montant_Token_Estime__c);
  }
  get fraisGestion() {
    return fmt(this.contrat.Frais_Gestion__c);
  }
  get typeFacturation() {
    return this.contrat.Type_Facturation__c || "—";
  }

  get hasTokenDates() {
    return !!(
      this.contrat.Date_Debut_Token__c || this.contrat.Date_Fin_Token__c
    );
  }
  get periodeToken() {
    return (
      fmtDate(this.contrat.Date_Debut_Token__c) +
      " → " +
      fmtDate(this.contrat.Date_Fin_Token__c)
    );
  }
