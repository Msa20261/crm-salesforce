...
export default class CartographieOpportunites extends LightningElement {
  ...
  get optionsEtapes() {
    const stages = [
      ...new Set(this._allOpps.map((o) => o.stageName).filter(Boolean))
    ].sort();
    return [
  ...
  get optionsAnnees() {
    const years = [
      ...new Set(this._allOpps.map((o) => o.anneeCreation).filter(Boolean))
    ].sort((a, b) => b - a);
    return [
  ...
  get filteredOpps() {
    return this._allOpps.filter((o) => {
      ...
      if (this.filtreAnnee && String(o.anneeCreation) !== this.filtreAnnee)
        return false;
      return true;
    });
  }
  ...
  handleFiltreCompte(event) {
    this.filtreCompte = event.detail.value;
  }
  handleFiltreEtape(event) {
    this.filtreEtape = event.detail.value;
  }
  handleFiltreAnnee(event) {
    this.filtreAnnee = event.detail.value;
  }
