...
export default class ConsumptionTimeline extends LightningElement {
  ...
  get periodes() {
    return this._data.map((c) => {
      ...
      return {
        ...
        cssSource:
          "source-badge " +
          (c.Source__c === "Ludwig API" ? "source-api" : "source-manuel")
      };
    });
  }
  ...
  // Totaux globaux
  get totalAchete() {
    return fmt(this._data.reduce((s, c) => s + (c.Montant_Achete__c || 0), 0));
  }
  get totalConso() {
    return fmt(
      this._data.reduce((s, c) => s + (c.Montant_Consomme__c || 0), 0)
    );
  }
  get totalRestant() {
    return fmt(this._data.reduce((s, c) => s + (c.Montant_Restant__c || 0), 0));
  }

  get pctGlobal() {
    const achete = this._data.reduce(
      (s, c) => s + (c.Montant_Achete__c || 0),
      0
    );
    const conso = this._data.reduce(
      (s, c) => s + (c.Montant_Consomme__c || 0),
      0
    );
