...
export default class PopupRdv extends LightningElement {
    ...
    get statutOptions()  { return STATUT_OPTIONS;  }
    get prioriteOptions(){ return PRIORITE_OPTIONS; }

    // ── MODAL RDV — OUVERTURE / FERMETURE ─────────────────────────────────────

    ouvrirModal() { this.modalOuvert = true;  this.erreur = null; }
    fermerModal() { this.modalOuvert = false; this._reinitRdv(); }
    ...
    handleHeureDebut(e)  { this.heureDebut   = e.detail.value; }
    handleHeureFin(e)    { this.heureFin     = e.detail.value; }
    ...
    async handleSubmit() {
        ...
        try {
            ...
            this.dispatchEvent(new ShowToastEvent({
                ...
                message: 'L\'Event (calendrier) et la Task ont été créés. Emails envoyés.',
                variant: 'success'
            }));
            this.fermerModal();
        } catch (e) {
    ...
    handleDateRdvCR(e)         { this.dateRdvCR           = e.detail.value; }
    handleTypeRdvCR(e)         { this.typeRdvCR           = e.detail.value; }
    handleAvecQui(e)           { this.avecQui             = e.detail.value; }
    handleInteretDetecte(e)    { this.interetDetecte      = e.detail.value; }
    handleNotesRdv(e)          { this.notesRdv            = e.detail.value; }
    handleProchaineAction(e)   { this.prochaineAction     = e.detail.value; }
    handleDateProchaineCR(e)   { this.dateProchaineAction = e.detail.value; }
    ...
    async handleSubmitCR() {
        ...
        try {
            ...
            this.erreurCR = e.body?.message || 'Une erreur est survenue. Veuillez réessayer.';
        } finally {
