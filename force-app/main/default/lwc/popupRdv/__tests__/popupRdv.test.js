...
jest.mock(
  "@salesforce/apex/RdvController.creerRdv",
  () => ({ default: jest.fn() }),
  { virtual: true }
);
jest.mock(
  "@salesforce/apex/RdvController.creerCompteRendu",
  () => ({ default: jest.fn() }),
  { virtual: true }
);

const getDernierRdvLeadAdapter = registerApexTestWireAdapter(getDernierRdvLead);
const getUsersAdapter = registerApexTestWireAdapter(getUsers);
...
describe("c-popup-rdv", () => {
  ...
  it("charge les utilisateurs au montage via wire getUsers", async () => {
    ...
    getUsersAdapter.emit(MOCK_USERS);
  ...
  it("charge le dernier RDV du lead via wire getDernierRdvLead", async () => {
    ...
    getDernierRdvLeadAdapter.emit(MOCK_DERNIER_RDV);
    await Promise.resolve();
  ...
  it("ouvre la modal RDV au clic sur le bouton de création", async () => {
    ...
    const openBtn = element.shadowRoot.querySelector(
      '[onclick*="ouvrirModal"], .btn-new-rdv, button'
    );
    if (openBtn) {
      openBtn.click();
      await Promise.resolve();
  ...
  it("creerRdv est appelé avec les bons paramètres à la soumission du formulaire", async () => {
    ...
    const submitBtn = element.shadowRoot.querySelector(
      '[onclick*="soumettreRdv"], [type="submit"]'
    );
    if (submitBtn) {
      submitBtn.click();
      await Promise.resolve();
  ...
  it("creerCompteRendu est appelé à la soumission du formulaire CR", async () => {
    ...
    const submitCrBtn = element.shadowRoot.querySelector(
      '[onclick*="soumettreCompteRendu"]'
    );
    if (submitCrBtn) {
      submitCrBtn.click();
      await Promise.resolve();
